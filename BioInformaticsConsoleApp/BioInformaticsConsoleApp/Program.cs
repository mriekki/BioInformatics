...
//            return count(S, m - 1, n) + count(S, m, n - S[m - 1]);
        ...
        public static int count2(List<int> S, int index, int mass)
        {
            // table[i] will be storing the number of solutions for
            // value i. We need n+1 rows as the table is constructed
            // in bottom up manner using the base case (n = 0)
            int[] table = new int[mass + 1];
        ...
        public static int count3(List<int> S, int m, int n)
        {
            ...
            for (int i = 1; i < n + 1; i++)
            {
                for (int j = 0; j < m; j++)
                {
                    if (i == 0)
                        ...
                        table[i, j] = table[i - S[j], m] + table[i, j - 1];
